...
selected = option_menu(
    ...
    icons=["person", "file-earmark-text", "bar-chart"],
...
if selected == "Portfolio":
    ...
    if not st.session_state.project_opened:
        st.title("Projects")
        name = st.selectbox("Select a project:", [""] + list(PROJECTS.keys()))
        
        if name != "":
            # Special logic for Factor Analysis: Run immediately
            if name == "Factor Analysis (Python)":
        ...
        if p["type"] == "python_script":
            ...
            st.image(APP_DIR / p["image_path"], use_container_width=True)
        elif p["type"] == "shiny":
